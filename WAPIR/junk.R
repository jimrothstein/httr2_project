## HTTR2-pagination.qmd
## JR modified (2024-04-24)
## PURPOSE:   practice pagination
---
engine: knitr
...
names(candidates_single) # [1] "api_version" "pagination"  "results"    
length(candidates_single$results)  #> [1] 20  # this page
candidates_single$pagination$count #> [1] 173

# Examine, counts, pages, ...
...
saveRDS(
  candidates_multi, 
  here::here("slides", "httr2", "pagination", "candidates_multi.rds")
)
```
:::
::::
...
::: {.column .fragment}
``40`{r}
n#| label: httr2-pagination-crossref-multi
crossref_multi <- 
  crossref_request |> 
  req_retry(max_tries = 3) |> 
5  req_perform_iterative(
    next_req = iterate_with_offset(
      "offset",
      resp_pages = \(resp) {
        content <- resp_body_json(resp)
...
saveRDS(
  mtg_cards_multi, 
  here::here("slides", "httr2", "pagination", "mtg_cards_multi.rds")
)
```
:::
::::
...
saveRDS(
  dnd_multi, 
  here::here("slides", "httr2", "pagination", "dnd_multi.rds")
)
```
:::
::::
