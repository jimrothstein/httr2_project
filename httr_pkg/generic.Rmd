...
# =============================
    ## assume type = "comments"
# =============================


    query_params$base_url  <- "https://www.googleapis.com/youtube/v3/commentThreads"
    query_params$part   <-   "snippet,replies"

    query_params$fields  <-  paste(sep = ", ", "nextPageToken",
      "items(snippet(topLevelComment(snippet(videoId,textDisplay))))"
    )

    query_params$videoId  = "Mec9sw1cJk8"  # carter

    ## no playlistId
    query_params["playlistId"]  <- NULL
    #query_params$playlistId  =  "PLlXfTHzgMRUIqYrutsFXCOmiqKUgOgGJ5"  # Pavel Grinfeld, Linear Alg 3

    ## and no channelId
    query_params["channelId"]  <- NULL
...
process  <- function(r = NULL, type = NULL, db = NULL){
      ...
      ## rbind is done aftert this call; not in this function
      ...
      if (type == "comments") {
          db  <- json_content$items$snippet$topLevelComment$snippet
...
## run 1st time (may refresh stale token)
      ...
      r  <- process(r=r, type="comments", db = db)
      db <- r$db
